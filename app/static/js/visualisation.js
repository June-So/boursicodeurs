       ...
       $.ajax({
           ...
           success: function(prices) {
            ...
            var dateFormat = d3.timeParse("%Y-%m-%d %h:%m:%s");
                ...
                for (var i; i <  Object.keys(prices).length; i++) {
                    console.log(price[i][0][0].Date);
                    date_format = dateFormat(price[i]['Date']);
                    price[i]['Date'] = date_format;
                    //dates.push(new Date(price['Date']));
                };

                /* param zone visualisation */
                const margin = {top: 15, right: 65, bottom: 205, left: 50},
                w = 1000 - margin.left - margin.right,
                h = 625 - margin.top - margin.bottom;

                /* init zone de visualisation */
            ...
            //var xmin = d3.min(prices, function(d) { return d.value; });
            //var xmax = d3.max(prices, function(d) { return d.value; });
            //var xScale = d3.scaleLinear().domain([-1, dates.length])
            //				.range([0, w])
            //var xDateScale = d3.scaleQuantize().domain([0, dates.length]).range(dates)
            //et xBand = d3.scaleBand().domain(d3.range(-1, dates.length)).range([0, w]).padding(0.3)
                console.log(dates)
                ...
                /* Y Axis */
                var ymin = d3.min(prices.products.map(r => r.Low));
                var ymax = d3.max(prices.products.map(r => r.High));
